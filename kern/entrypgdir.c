...
pte_t entry_pgtable[NPTENTRIES * 4];
pte_t entry_gpio[NPTENTRIES];
...
pde_t entry_pgdir[NPDENTRIES] = {
	...
	[0]
	        = ((uintptr_t)entry_pgtable - KERNBASE) + PDE_P,
	[1]
	        = ((uintptr_t)(&entry_pgtable[256]) - KERNBASE) + PDE_P,
	[2]
	        = ((uintptr_t)(&entry_pgtable[512]) - KERNBASE) + PDE_P,
	[3]
	        = ((uintptr_t)(&entry_pgtable[768]) - KERNBASE) + PDE_P,

	// Map VA's [GPIO_BASE, GPIO_BASE+1MB)
	[PDX(GPIO_BASE)]
	        = ((uintptr_t)entry_gpio - KERNBASE) + PDE_P,
...
__attribute__((__aligned__(PGSIZE)))
pte_t entry_gpio[NPTENTRIES] = {
    [0] = GPIO_BASE | PTE_P | PTE_W,
    [PTX(AUX_BASE)] = AUX_BASE | PTE_P | PTE_W, 
};
